...
const generateAlphabetOptions = (): Option[] => {
    const alphabet = "ABCDEFGHIJKLMNOPQRSTUVWXYZ";
    return alphabet.split('').map(letter => ({ id: letter, name: letter }));
};

const alphabetSegmentColors = ["#f7b71d", "#fdef96"];
const alphabetTextColors = ["#FFFFFF", "#000000"];

export default function AlphabetWheelClient({ translations, lang }: AlphabetWheelProps) {
  ...
  return (
    <>
      <WheelPageClient
        lang={lang}
        translations={t}
        storageKeySuffix="alphabet"
        initialOptions={generateAlphabetOptions()}
        pageTitle={t.pageTitle}
        pageDescription={t.description}
        showOptionsPanel={false}
        wheelColors={alphabetSegmentColors}
        textColor={alphabetTextColors}
        textStrokeColor="#000000"
        segmentFontSize={18}
        textRadiusRatio={0.75}
      />
